#!/usr/bin/env bun
/**
 * Build Script for @clockwork-engine/platform-web-pixi
 *
 * Builds the package for both Node.js and browser environments:
 * 1. TypeScript compilation (for Node.js/NPM)
 * 2. Browser bundles (development + production) for browser tests
 */
...
console.log("🏗️  Building @clockwork-engine/platform-web-pixi...\n")
...
console.log("   ✅ platform-web-pixi.js")
console.log("   ✅ platform-web-pixi.js.map")
...
const prodBuild = await Bun.build({
  entrypoints: [entrypoint],
  outdir: DIST_DIR,
  naming: "platform-web-pixi.min.js",
  target: "browser",
  format: "esm",
  minify: true,
  sourcemap: "external",
...
console.log("   ✅ platform-web-pixi.min.js")
console.log("   ✅ platform-web-pixi.min.js.map")

// Show bundle sizes
console.log("\n📊 Bundle sizes:")
const devStats = await Bun.file(
  path.join(DIST_DIR, "platform-web-pixi.js"),
).stat()
const prodStats = await Bun.file(
  path.join(DIST_DIR, "platform-web-pixi.min.js"),
).stat()
